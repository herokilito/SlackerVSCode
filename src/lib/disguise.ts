import type { Chapter, DisguiseLine } from '@renderer/types'
...
function fakeChapterFilename(chapterIndex: number, bookId: string): string {
  const rng = mulberry32(hashStr(bookId + ':ch:' + chapterIndex))
  const num = String(chapterIndex + 1).padStart(2, '0')
  const prefixes = [
    'chapter', 'ch', 'module', 'section', 'part', 'step', 'stage', 'phase', 'page', 'view'
  ]
  const names = [
    'handler', 'parser', 'processor', 'service', 'controller', 'manager', 'utils', 'helper',
    'model', 'config', 'index', 'main', 'core', 'base', 'types', 'impl', 'spec', 'test',
    'route', 'store', 'hook', 'provider'
  ]
...
function fakeDirName(bookId: string, groupIndex: number): string {
...
function fakeProjectName(bookId: string): string {
...
function pickImports(rng: () => number, count: number): string[] {
  const out: string[] = []
...
function pickCodeBlock(rng: () => number): string[] {
...
export function buildDisguisedLines(bookId: string, chapterIndex: number, chapter: Chapter): DisguiseLine[] {
  const rng = mulberry32(hashStr(bookId + ':render:' + chapterIndex))
  const out: DisguiseLine[] = []
